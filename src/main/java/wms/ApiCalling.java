...
//        queryParams2.put("end_time", endTimeEpoch);
    ...
    private static List<Map<String, String>> searchStaffAllocation(Map<String, String> cookies) {
        ...
        do {
            ...
            Response searchStaffAllocation = requestSpec(cookies)
                    .queryParams(queryParams)
                    .get("/api/v2/apps/dashboard/labor/dsstaff/search_staff_allocation");

            if (searchStaffAllocation.getStatusCode() != 200) {
                throw new RuntimeException("Failed to search Staff Allocation. API status code "
                        + searchStaffAllocation.getStatusCode());
            }

            if (pageNo == 1) {
                Integer totalObj = searchStaffAllocation.jsonPath().get("data.total");
                total = (totalObj != null) ? totalObj : 0;
            }

            List<Map<String, Object>> rawList = searchStaffAllocation.jsonPath().getList("data.list");
    ...
    public static int[] countPickerPacker(String warehouse) {
        ...
        for (Map<String, String> staff : staffAllocation) {
            if (staff.get("act_name").equalsIgnoreCase("Picking")) {
                picker += 1;
            } else if (staff.get("act_name").equalsIgnoreCase("CheckingWhilePacking")) {
                packer += 1;
            }
        }
